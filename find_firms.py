...
async def compile_investment_memo(company):
    ...
    direct_gen_outline_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are a private equity analyst. Create a comprehensive market analysis outline about a user-provided "
                "investment opportunity. Include sections on target companies in this sector, market analysis, competitive landscape, "
    ...
    gen_related_topics_prompt = ChatPromptTemplate.from_template(
        """I'm writing an Investment memo for an investment opportunity mentioned below. Please identify and recommend 
        some web pages on closely related subjects and investment opportunities. I'm looking for examples that provide insights 
        into interesting aspects commonly associated with this topic and sector, or examples that help me 
        understand the typical content and structure included in investment theses for similar companies and opportunities.
    ...
    gen_perspectives_prompt = ChatPromptTemplate.from_messages([
        (
            ...
            Each analyst represents a different perspective, role, or affiliation related to this investment opportunity and topic.
    ...
    gen_qn_prompt = ChatPromptTemplate.from_messages([
        (
            ...
            Ask ONE question at a time about the target investment opportunity, but ensure each question is directly related to your specific expertise; i.e.  {description}.
    ...
    gen_answer_prompt = ChatPromptTemplate.from_messages(
     ...
     to write an Investment memo on a topic you know. You have gathered the related information and will now use the information to form a response.
    ...
    class CompanySearchFilters(BaseModel):
        ...
        min_revenue_millions: Optional[float]
        max_revenue_millions: Optional[float]
        employee_range: Optional[Tuple[int, int]] = None
        exit_intent: Optional[str] = None
        # Add any other fields you need

    parse_filters_prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            "You are an AI that extracts structured filters from a user request about finding companies.\n"
            "Output valid JSON that fits this schema:\n{schema}"
        ),
    ...
    parse_filters_chain = parse_filters_prompt | llm.with_structured_output(
        CompanySearchFilters,
        include_raw=True
    )

    def parse_user_query_to_filters(user_input: str) -> CompanySearchFilters:
        parsed = parse_filters_chain.invoke(
            {
                "messages": [HumanMessage(content=user_input)],
                "schema": CompanySearchFilters.schema_json()
            }
        )
        return parsed["parsed"]

    def build_search_query(filters: CompanySearchFilters) -> str:
        # Example of turning filters into a search query
        parts = []
        if filters.industry:
            parts.append(filters.industry)
        if filters.location:
            parts.append(filters.location)
        if filters.min_revenue_millions:
            parts.append(f"revenue > {filters.min_revenue_millions} million")
        if filters.max_revenue_millions:
            parts.append(f"revenue < {filters.max_revenue_millions} million")
        if filters.employee_range:
            low, high = filters.employee_range
            parts.append(f"{low}-{high} employees")
        if filters.exit_intent:
            parts.append(filters.exit_intent)
        # Combine into a single string
        return " ".join(parts) if parts else "Companies to invest in"

    import asyncio
    from langchain_community.tools.tavily_search import TavilySearchResults
    from langchain_community.utilities.duckduckgo_search import DuckDuckGoSearchAPIWrapper

    # Our search engine wrappers
    tavily_search = TavilySearchResults(api_key="TAVILY_API_KEY", max_results=10)
    duckduckgo = DuckDuckGoSearchAPIWrapper()

    async def search_companies_textual(query: str) -> list:
        """
        Tries Tavily. If no results or fail, fallback to DuckDuckGo.
        Returns a list of results as [{'content': '...', 'url': '...'}, ...].
        """
        try:
            results = tavily_search.invoke(query)
            if results:
            ...
            ddg_results = duckduckgo._ddgs_text(query)
    ...
    extract_companies_prompt = ChatPromptTemplate.from_template("""
    You have a user looking for companies with these filters:
    {filters}

    Below are search results from the internet:

    {search_results}

    Identify any companies mentioned in the text that match the user’s filters. 
    ...
    async def extract_companies(filters: CompanySearchFilters, raw_results: list) -> List[dict]:
        # Compose the textual input
        combined_text = ""
        for idx, res in enumerate(raw_results, start=1):
            snippet = res['content'][:1000]  # maybe limit content
            combined_text += f"\n\n[Source {idx}]: {snippet}"

        prompt_text = extract_companies_prompt.format(
            filters=filters.json(),
            search_results=combined_text
        )
        # Now call your LLM
        extraction_llm = ChatOpenAI(model="gpt-4")
        ...
        try:
            ...
            # fallback: return an empty list or parse in some other way
            return []

    class FindCompaniesToolInput(BaseModel):
        """Subset of fields we need from the overall workflow state."""
        topic: str  # We'll treat this as the user query
        # You can add more fields if your state includes them, e.g.:
        # outline: Optional[Dict[str, Any]] = None
        # or interview_results: Optional[List[Any]] = None
        # etc.

    @tool
    async def find_companies_with_web_search(data: FindCompaniesToolInput) -> str:
        """
        This tool is now designed to accept the entire state (or a subset)
        as `data`, which must match the `FindCompaniesToolInput` model.
        """
        # Step A: Convert the "topic" into filters
        # (Assumes you still have parse_user_query_to_filters, build_search_query, etc.)
        filters = parse_user_query_to_filters(data.topic)
        query = build_search_query(filters)

        # Step B: Do the actual search
        raw_results = await search_companies_textual(query)

        # Step C: Extract structured info from the raw search results
        matches = await extract_companies(filters, raw_results)
    ...
    refine_outline_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are a Private Equity Investor. You have gathered information from experts and search engines. 
                Now, you are refining the outline of the Investment memo. \
    ...
    writer_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are an expert Private Equity Investor and an expert Investment memo author. Write the complete "
                "Investment memo on an investment in {topic} using the following section drafts:\n\n"
    ...
    nodes = [
        ("find_targets", find_companies_with_web_search),
...
st.write("Describe the target investment opportunity you are researching:")
